...
// Mirrors upsertParticipant.cjs, but against the independent
// playoffs_participants table -- kept as its own function (rather than a
// table-name param on the existing one) so playoffs roster changes can
// never accidentally touch the regular-season participants table.
exports.handler = async (event) => {
  try {
    ...
    const { error } = await admin.from("playoffs_participants").upsert([row], { onConflict: "user_name" });
