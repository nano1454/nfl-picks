...
// Mirrors setParticipantActive.cjs, but against playoffs_participants.
exports.handler = async (event) => {
  try {
    ...
    const { error } = await admin.from("playoffs_participants").update({ active }).eq("user_name", user_name);
